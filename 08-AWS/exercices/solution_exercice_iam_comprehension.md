...
1. Un membre du groupe télécharge (`s3:GetObject`) le fichier `rapports/ventes-2026.csv` du bucket `formation-documents`. Autorisé ou refusé ? Autorisé
2. Le même membre télécharge `confidentiel/salaires.xlsx`. Autorisé ou refusé ? refusé Quel statement s'applique en dernier ressort ?  InterdireConfidentiel
3. Il tente de **supprimer** (`s3:DeleteObject`) le fichier `rapports/ventes-2026.csv`. Autorisé ou refusé ? refusé S'agit-il d'un deny explicite ou d'un deny implicite ? deny implicite
4. Il liste le contenu du bucket (`s3:ListBucket`). Peut-il **voir dans la liste** que le dossier `confidentiel/` existe et les noms des fichiers qu'il contient ? Oui il voit les noms
5. Un administrateur ajoute plus tard au groupe une politique `AmazonS3FullAccess` (gérée AWS, qui autorise `s3:*` sur `*`). Le téléchargement de `confidentiel/salaires.xlsx` devient-il possible ? Non toujours refusé Pourquoi ? A cause de InterdireConfidentiel
...
1. `stagiaire-ops` lance une instance `t3.micro` dans eu-west-3. Autorisé ou refusé ? Autorisé avec LancerPetitesInstancesParisSeulement
2. Il lance une instance `t3.micro` dans eu-central-1 (Francfort). Autorisé ou refusé ?refusé Est-ce un deny explicite ? non implicite
3. Il lance une instance `m5.large` dans eu-west-3. Autorisé ou refusé ? refusé
4. Il exécute `aws ec2 describe-instances --region us-east-1`. Autorisé ou refusé ? Autorisé avec LectureEC2Partout
5. Il tente d'**arrêter** (`ec2:StopInstances`) une instance existante à Paris. Autorisé ou refusé ? refusé
6. Les deux clés de la `Condition` (`aws:RequestedRegion` et `ec2:InstanceType`) sont dans le **même** bloc `StringEquals`. L'instance doit-elle satisfaire les deux conditions, ou une seule suffit-elle ? Les deux conditions se combine ( AND )
...
1. L'auditeur (avec `ReadOnlyAccess`) peut-il lister les utilisateurs IAM du compte ? OUI Peut-il en **créer** un ? NON
2. L'auditeur peut-il télécharger un objet S3 ? OUI Peut-il en téléverser (`s3:PutObject`) un ? NON
3. Citez deux avantages concrets d'une politique **gérée** (AWS ou client) par rapport à une politique **inline**.
 - politique gérée et réutilisable
 - Modifiable 
 - Visible et auditable
4. La politique inline de `dupont` autorise `s3:*` sur les objets du bucket. Peut-il pour autant **supprimer le bucket** lui-même (`s3:DeleteBucket`) ? Regardez bien le `Resource`. NON 
5. En audit, pourquoi les politiques inline sont-elles plus difficiles à repérer que les politiques gérées ?
  - Elle ne figure pas avec les autres policies
 Quel risque le `Sid` de celle-ci révèle-t-il sur les pratiques de l'équipe ? 
  - utilisation d'un potique qui ne devrais pu exister (temporaire)
